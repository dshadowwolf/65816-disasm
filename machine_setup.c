...
void initialize_machine(machine_state_t *machine) {
    ...
    region1->start_offset = 0x8000;
    region1->end_offset = 0xFFFF;
    region1->data = (uint8_t *)malloc(32768 * sizeof(uint8_t));
    region1->read_byte = read_byte_from_region_nodev;  // Default read/write functions can be set later
    region1->write_byte = write_byte_to_region_nodev;
    region1->read_word = read_word_from_region_nodev;
    region1->write_word = write_word_to_region_nodev;
    region1->flags = MEM_READONLY;
